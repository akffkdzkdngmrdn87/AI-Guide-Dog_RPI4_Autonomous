...
시스템을 구동하기 전, 반드시 `필수_하드웨어_최적화_가이드.md`를 참고하여 ZRAM 및 커널 파라미터 설정을 완료해야 합니다.

### ⚙️ [사전 준비] 라이다(LiDAR) 공식 패키지 빌드
초기 1회만 수행하면 되는 SLLidar 패키지 클론 및 빌드 작업입니다.

```bash
# 라이다(LiDAR) 구동을 위한 공식 패키지 다운로드 및 빌드
cd ~/ros2_ws/src
git clone -b ros2 [https://github.com/Slamtec/sllidar_ros2.git](https://github.com/Slamtec/sllidar_ros2.git)
cd ~/ros2_ws
